use super::Plugin;
use futures::{self, future, prelude::*, stream::futures_unordered, Future, Stream};
...
pub fn create_plugin_monitor(
    ...
    // If no plugins present
    // Create a future that just polls the shutdown signal
    if plugins.is_empty() {
        let poll_abort = future::poll_fn(move || -> Poll<(), io::Error> {
            match abort_signal.load(Ordering::Relaxed) {
                true => Ok(Async::Ready(())),
                false => Ok(Async::NotReady),
            }
        });
        return boxed_future(poll_abort);
    }
    ...
    let f = futures_unordered(plugin_monitors).into_future().then(move |res| {
        ...
        remaining
            ...
            .collect()
            .map(|_| ())
            .map_err(|_: ()| io::Error::new(io::ErrorKind::Other, "Placeholder"))
    });
